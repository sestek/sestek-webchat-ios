...
public extension HubConnection {
    ...
    func on<T1: Decodable, T2: Decodable, T3: Decodable>(method: String, callback: @escaping (_ arg1: T1?, _ arg2: T2?, _ arg3: T3?) -> Void) {
        let cb: (ArgumentExtractor) throws -> Void = { argumentExtractor in
            ...
            var secondArg: T2?
            var thirdArg: T3?
            if let arg1 = try? argumentExtractor.getArgument(type: T1.self) {
                firstArg = arg1
            }
            if let arg2 = try? argumentExtractor.getArgument(type: T2.self) {
                secondArg = arg2
            }
            if let arg3 = try? argumentExtractor.getArgument(type: T3.self) {
                thirdArg = arg3
            }
            callback(firstArg, secondArg, thirdArg)
        }
